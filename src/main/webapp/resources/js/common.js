...
var codMap,lofMap,gocTicMap,gocMapDlv;
var shopMaps = {};
var shopList = {};
var _ajax = $.ajax;
$.ajax = function(conf){
		...
		fnDrawCallback: function( oSettings ) {
			...
			if(document.querySelector('#actionSms') && !document.querySelector('#actionSms').getAttribute('hasEvent')){
				document.querySelector('#actionSms').addEventListener('click',(obj)=>{
					var param = {
						cuiNums : obj.target.getAttribute('data-val').split(","),
...
<c:if test="${empty pk}">
		...
		for(let key in conf.dtConf.columns){
			...
			if(conf.dtConf.columns[key].type==='co'){
				...
				conf.dtConf.columns[key].render = function(data,type,row){
					return '<img src="img?fileName=' + data + '" width="100px" height: "auto"/>';
				}
			}
		}
		...
		if(searchObj){
			...
			if(document.querySelectorAll('[id$="lofSido"]').length!==0){
				document.querySelectorAll('[id$="lofSido"').forEach((obj,idx)=>{
					obj.addEventListener('change',function(){
						...
						if(targetObj){
							targetObj.innerHTML = '';
							var subHtmlCod = '<option value="">선택</option>';
			...
			document.querySelectorAll('select[data-cods]').forEach((obj,idx)=>{
				...
				let htmlCode = '<option value="">선택</option>';
				if(window.shopMaps[cods[0]]){
					...
					obj.onchange = function(){
						if(document.querySelectorAll('select[id^="' + commonId + '"]').length!=0){
							document.querySelectorAll('select[id^="' + commonId + '"]').forEach((tObj,idx)=>{
								if(obj!=tObj && cods[1]<tObj.getAttribute('data-cods').split(',')[1]){
									tObj.innerHTML = '<option value="">선택</option>';
								}
							});
						}
						...
						if(subObj){
							let subHtmlCode = '<option value="">선택</option>';
			...
			document.querySelectorAll('select[data-cods]').forEach((obj,idx)=>{
				...
				if(window.shopMaps[cods[0]] && depth==1){
					for(let key1 in  window.shopMaps[cods[0]]){
						htmlCode += '<option value="' + key1 +'">' + key1 + '</option>';
					}
					obj.innerHTML = htmlCode;
					obj.onchange = function(){
						if(document.querySelector('input[data-target*="' + obj.id + '"]')){
							document.querySelector('input[data-target*="' + obj.id + '"]').value = '';
						}
						let subId = prefix + surfix + (depth + 1);
						let subObj = document.querySelector('select#' + subId + '[data-cods]');
						let subVal = this.value;
						htmlCode = '<option value="">선택</option>';
						for(let key2 in  window.shopMaps[cods[0]][subVal]){
							htmlCode += '<option value="' + key2 +'">' + key2 + '</option>';
						}
						subObj.innerHTML = htmlCode;
						subObj.onchange = function(){
							if(document.querySelector('input[data-target*="' + obj.id + '"]')){
								document.querySelector('input[data-target*="' + obj.id + '"]').value = '';
							}
							let endId = prefix + surfix + (depth + 2);
							let endObj = document.querySelector('select#' + endId + '[data-cods]');
							let endVal = this.value;
							htmlCode = '<option value="">선택</option>';
							for(let key3 in  window.shopMaps[cods[0]][subVal][endVal]){
								htmlCode += '<option value="' + key3 +'">' + key3 + '</option>';
							}
							endObj.innerHTML = htmlCode;
							if(endObj.getAttribute('data-target')){
								endObj.onchange = function(){
									if(document.querySelector('#' + endObj.getAttribute('data-target'))){
										console.log(endVal);
										console.log(this.value);
										console.log(window.shopMaps[cods[0]][subVal][endVal][this.value]);
										try{
											document.querySelector('#' + endObj.getAttribute('data-target')).value = window.shopMaps[cods[0]][subVal][endVal][this.value];
										}catch(e){
											document.querySelector('#' + endObj.getAttribute('data-target')).value = '';
										}
									}
								};
							}
					...
					obj.innerHTML = '<option value="">선택</option>';
				}
			});
			*/
...
$(document).ready(function(){
	...
	document.querySelectorAll('select[data-cod]').forEach((obj,idx)=>{
		...
		let htmlCod = '<option value="">선택</option>';
		if(cod==='lofMap'){
			...
			htmlCod='<option value=""> 선택</option>';
